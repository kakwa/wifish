...
install:
	...
	cp src/wifish-cfg src/wifishd $(DESTDIR)/$(INSTALL_DIR)/
	sed -i "s|^DEFAULT_CONFIG_FILE.*|DEFAULT_CONFIG_FILE='$(SYSCONF_DIR)/wifish/wifish.conf'|" $(DESTDIR)/$(INSTALL_DIR)/wifish-cfg $(DESTDIR)/$(INSTALL_DIR)/wifishd
	...
	chmod 755 $(DESTDIR)/$(INSTALL_DIR)/wifish-cfg $(DESTDIR)/$(INSTALL_DIR)/wifishd
